...
local DarkraiX = loadstring(game:HttpGet("https://pastebin.com/raw/qFJQn7q2"))()

local Library = DarkraiX:Window("星云脚本" , "您目前使用的注入器:"..identifyexecutor(), "", Enum.KeyCode.RightControl);

local Announcement = Library:Tab("公告",'6035145364')

Announcement:Seperator("星云脚本欢迎您的使用，你好"..game.Players.LocalPlayer.Name.."")

Announcement:Seperator("当前游戏ID为:" .. game.GameId .. ".")

Announcement:Seperator("本脚本将持续云端更新")

Announcement:Seperator("当前的脚本版本:3.5")

Announcement:Seperator("更新日志:")

Announcement:Seperator("无")

Announcement:Seperator("by 日月星辰工作室")

local Player = Library:Tab("玩家/通用")
...
local FM1 = Library:Tab("伐木大亨2",'6035145364')
...
local function donate(plr, amount)
    ...
    notify('星云', '转钱' .. tostring(amount) .. ' 给 ' .. tostring(plr), 4);
    scr2.exitNotice();
end
...
Section:Button("获取当前死亡后加载时间", function()
    bailib:Notification('星云', " 时间为" .. bai.loaddupeaxewaittime .. "秒", "好")
end)
...
Sectionhuanjin:Button("船离开时间", function()
    ...
    if game.Workspace.Ferry.TimeToDeparture.Value == 0 then
        ...
        notify('星云', "距离船离开时间有" .. game.Workspace.Ferry.TimeToDeparture.Value .. "秒", 4)
    end
end)
Sectionhuanjin:Button("桥上升时间", function()
    ...
    if game.Workspace.Bridge.DownTime.Value < 0 then
        ...
        notify('星云', "距离桥上升的时间有" .. game.Workspace.Bridge.DownTime.Value .. "秒", 4)
    end
end)
Sectionhuanjin:Button("查看游戏时间", function()
    notify('星云', "游戏时间为" .. tostring(game.Lighting.TimeOfDay):sub(1, -2):sub(1, -2):sub(1, -2), 4)
end)
...
FM:Button("bark2.0", function()
 ...
 FM:Button(" 星云  -- 伐木大亨2(盗版浮光掠影)", function()
...
local Doors = Library:Tab("Doors［🚪］",'6035145364')

local tool = Doors:section("「脚本」",true)

Doors:Button("最强脚本", function()
...
local Ple = Library:Tab("监狱人生")

local JYR = Ple:Seperator("非传送类")
...
local Quick = Library:Tab("极速传奇")

local JSR = Quick:Seperator("卡宠/刷经验")
...
local Bee = Library:Tab("蜂群模拟器")

local JBH = Bee:Button("HUA蜂群", function()
...
local JBH = creds:section("「脚本」",true)
...
local Dls = Library:Tab("超级大力士模拟器",'6035145364')
...
local Script = Library:Tab("其他各大脚本")
...
local About = Library:Tab("关于我们")

About:Seperator("作者：云")

Statement = Library:Tab("声明")

Statement:Seperator("本脚本永远免费")

Statement:Seperator("欢迎加入我们的群聊834451572")